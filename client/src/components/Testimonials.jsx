...
const Testimonial = () => {
    const dummyTestimonialData = [
        ...
        },
        {
            ...
            content: 'QuickAI has transformed our content creation process. The AI tools specifically the image generation tool have helped us produce high-quality content faster than ever before.',
            rating: 4,
        },
    ]
                ...
                {dummyTestimonialData.map((testimonial, index) => (
                    <div key={index} className='p-8 m-4 max-w-xs rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 hover:-translate-y-1 transition duration-300 cursor-pointer'>
                        <div className="flex items-center gap-1">
                            {Array(5).fill(0).map((_, index)=> (<img key={index} src={index < testimonial.rating ? assets
                            .star_icon : assets.star_dull_icon} className="w-4 h-4" alt="star"/>))}
                        </div>
